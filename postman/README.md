...
> ℹ️ **Variable `test_email`**: tiene default `postman+manual@postman-test.com` en **dos lados** — los 4 environments (`azure-swa`, `azure`, `local`, `node`) Y en la collection variable. **Postman da prioridad a la collection variable sobre la env variable** (cuando la collection var tiene un valor, aunque sea string vacío, gana). Por eso, si re-importás la collection, el default de la collection ya está y no necesitás tocar el env. Si querés un email único por corrida, sobreescribilo con `--env-var "test_email=postman+$(date +%s)@test.com"` en newman, o en Postman UI: click derecho en la collection → Edit → Variables. Los tests 02.1/02.2/02.4/02.5 tienen un guard que falla con mensaje claro si `test_email` queda vacío.
